...
const Profile = () => {
  ...
  // Список покупок будет выглядеть следующим образом: ["celery", "onion", "carrot", "orange"]
  ...
  // перейдите на страницу личного профиля, если имя пользователя ваше
  ...
  const handleDeleteMeal = async (idMeal) => {
    try {
      ...
      // после успеха удалите идентификатор блюда из localStorage
      removeMealId(idMeal);
    } catch (err) {
        ...
        {userData.savedRecipes.map((recipe) => (
          <Col className="recipe-card" lg={5}>
            ...
            <div>
              ...
              {/* кнопка добавления в корзину будет здесь */}
              {/* <button
